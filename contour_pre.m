    ...
    for p2 = (p1+1):numpar              % p2: index of 2nd varied parameter
        ...
        for i=1:maxi
            for j=1:maxj
                k(p1) = BETA(i,j);
                k(p2) = ALPHA(i,j);
                ED(i,j) = err_in_data(k);
            end
        end     
        z = z + 1;
        h = figure(z)
%         [C,g] = contour(BETA,ALPHA,ED,70)
        [C,g] = contour(BETA,ALPHA,ED,[25 50 100 250 500 1000 2000 4000])
%         clabel(C, g)
...
%         clabel(C, g, 'manual')
        ...
        saveas(h,strcat('mod',num2str(mymodel),'cont41',num2str(z)),'fig')
        saveas(h,strcat('mod',num2str(mymodel),'cont41',num2str(z)),'pdf')
    %     figure(2)
    %     surf(ALPHA,BETA,ED)
    end
end 
