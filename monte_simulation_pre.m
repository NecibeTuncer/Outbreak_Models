...
clc
format long
global mymodel tforward tmeasure initial_c iind Pdata true_params N
numiter = 1000;
...
num_para = length(true_params);     % number of parameters
total_ARE = zeros(6,num_para);
% N = sum(initial_c);
[~,y_trp] =ode45(@(t,y)(model_SIR(t,y,true_params)),tforward,initial_c);
X = zeros(num_para,numiter);
figure(1)
plot(tforward,y_trp,'LineWidth',1.5)
% title('trueparams = [0.5 0.2 0.1 0.1 0.1], iniSCIR = [20 1 1 0]')
...
%         set(gca,'YLim', [0 2]);
        ...
        hold off
        saveas(h,strcat('mod',num2str(mymodel),num2str(n-2),'noise', num2str(noisemag*100), 'fms'),'fig')
        saveas(h,strcat('mod',num2str(mymodel),num2str(n-2),'noise', num2str(noisemag*100), 'fms'),'pdf')
...
beep
export(dataset(Xcomp), 'file', 'estimates41.txt')
export(dataset(total_ARE), 'file', 'ARE41.txt')
